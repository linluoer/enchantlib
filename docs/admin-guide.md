...
### Q: EnchantLib 需要客户端安装吗?

**A**: 不需要。EnchantLib 是纯服务端模组。但自定义附魔的本地化名称需要客户端接受资源包(服务端自动推送)。

### Q: 修改附魔配置后如何生效?

**A**: 配置文件定义的附魔需**重启服务端**。`/enchantlib reload` 仅重载 `acquisition.toml` 的全局开关,不重载附魔定义。

### Q: 如何让玩家获得自定义附魔?

**A**: 三种途径:
1. **战利品注入**:探索箱子(地下城、矿道等)有概率获得附魔书
2. **村民交易**:图书管理员出售附魔书,或自定义村民交易
3. **管理员给予**:`/enchantlib give <player> <enchantment_id> [level]`(手持物品时直接附魔,空手时给附魔书)

### Q: `/enchantlib give` 是给附魔书还是直接附魔物品?

**A**: 取决于玩家是否手持物品:
- **手持物品**:直接对该物品附魔(若该物品支持该附魔)
- **空手**:给予一本带有该附魔的附魔书

### Q: 附魔之间如何互斥?

**A**: 通过互斥组(`exclusive_set`)实现。同组附魔不可共存于同一物品。可引用原版互斥组(如 `#minecraft:exclusive_set/damage`)或自定义互斥组。

### Q: 调试模式会影响性能吗?

**A**: 性能影响极小。生产环境建议关闭,仅排查问题时开启。

### Q: HTTP 服务器端口被占用怎么办?

**A**: 修改 `config/enchantlib/acquisition.toml` 中的 `http_server_port`,重启服务端。
...
**A**: 默认 `20`(每秒触发一次),推荐生产环境使用:
- `1` = 每 tick 触发(高精度但高 CPU)
- `20` = 每秒触发(平衡,推荐)
- `40+` = 每 2 秒以上触发(低开销)

使用实体 ID 相位偏移避免全服同一 tick 集中触发,即使设为较大值也不会出现"卡顿尖峰"。

> 注:此项仅控制 `BuiltInEvents.ENTITY_TICK`(基于装备附魔扫描)的触发间隔。
> 全局事件 `EnchantLibEvents.LIVING_ENTITY_TICK` 每 tick 触发,不受此配置控制
> (其懒挂载特性保证无回调时零开销)。

### Q: `trigger.toml` 是必须的吗?

**A**: 不是。`trigger.toml` 不存在时使用默认 `IGNORE` 策略(任何充能都触发)。仅当需要门控 `POST_ATTACK`/`PROJECTILE_HIT` 事件触发时配置。

### Q: 如何调整特定附魔的触发阈值?

**A**: 在 `trigger.toml` 中添加附魔级覆盖:
...
这样 `leech` 附魔在充能 < 0.7 时不触发,且效果按充能缩放。修改后需重启服务端。

### Q: 修改 `trigger.toml` 后如何生效?

**A**: 需重启服务端。`/enchantlib reload` 不重载触发策略配置。

### Q: 玩家报告"饱食度满但不回血",是 EnchantLib 的问题吗?

**A**: 可能是某个实现模组的 `FOOD_REGEN` 全局事件回调误判条件,错误取消了自然回血。
排查方式:
1. 查看是否安装了使用 `EnchantLibEvents.FOOD_REGEN` 的模组(常见于"静默契约"类压制回血附魔)
2. 让玩家脱下相关附魔装备测试
3. 临时移除实现模组验证

`FOOD_REGEN` 仅取消 `FoodData.tick` 内部的自然回血 heal 调用,**不影响**药水/金苹果/信标等
主动恢复手段,饱食度仍正常消耗。详见 7.3.7。

### Q: `/enchantlib debug status` 中的"事件回调覆盖附魔"为何比预期少?

**A**: 此统计仅含通过 `EnchantmentEventRegistrar` 注册的回调(基于装备附魔扫描的 `BuiltInEvents`)。
全局事件 `EnchantLibEvents.LIVING_ENTITY_TICK` / `FOOD_REGEN` 由实现模组直接通过
`Event.register` 注册,不进入此统计。可在启动日志中查关键字确认其启用状态。
